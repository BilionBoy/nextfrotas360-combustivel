...
export default function RelatoriosOperacionais() {
  ...
  const applyFilters = () => {
    ...
    if (filters.frotista) {
      data = data.filter((d) => d.frotista.toLowerCase().includes(filters.frotista.toLowerCase()))
    }
    if (filters.combustivel) {
      data = data.filter((d) => d.combustivel.toLowerCase().includes(filters.combustivel.toLowerCase()))
    }
    if (filters.dataInicio) {
      data = data.filter((d) => new Date(d.data) >= new Date(filters.dataInicio))
    }
    if (filters.dataFim) {
  ...
  const volumePorCombustivel = useMemo(() => {
    ...
    return Object.keys(grouped).map((key) => ({ name: key, volume: grouped[key] }))
  }, [filteredData])
  ...
  const handleExport = (format: string) => {
    toast({
      ...
      description: "Seu relatório está sendo gerado e o download começará em breve.",
    })
  }
  ...
  return (
    <div className="min-h-screen w-full bg-background p-4 md:p-8">
      ...
      >
        ...
        <p className="text-muted-foreground mb-8">Acompanhe o volume de vendas e a performance do seu posto.</p>
        ...
        <div className="bg-card border border-border rounded-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            />
            <Input name="dataInicio" value={filters.dataInicio} onChange={handleFilterChange} type="date" />
            <Input name="dataFim" value={filters.dataFim} onChange={handleFilterChange} type="date" />
          </div>
          ...
          <motion.div className="bg-card p-6 border border-border rounded-lg shadow-md">
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={volumePorCombustivel} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Tooltip cursor={{ fill: "rgba(13, 148, 136, 0.1)" }} />
                  <Legend />
                  <Bar dataKey="volume" fill="hsl(var(--secondary))" name="Volume (L)" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
